...
`DATA_DIR` and `DB_PATH` come from the environment (Docker sets both); the two result paths
are derived from `data_dir` unless a caller passes them explicitly, which is how `cli.py`
honours its `--out` flag. Result files therefore land next to the dataset -- repo root
locally, the mounted `/data` in Docker.
...
class Settings(BaseSettings):
    ...
    def _derive_result_paths(cls, values: Any) -> Any:
        """Point both result files at `data_dir` unless the caller named them itself."""
